...
def get_filters():
    ...
    while True:
        city = input("\n choose city : new york city , chicago ,washington \n").lower()#frist requirment
        if city not in ('new york city', 'chicago', 'washington'):
                print("choose one of the avaliable three cites")
                continue
        else:
             break
...
def Get_processeded(df):
    '''
    view 5 lines by 5 lines according to user input
    '''
    def get_more(more):
    ...
    low = 5
    referance = False
    while referance == False:
        more = input("\n get more 5 lines \n 'yes' or 'no' ")
        referance = get_more(more)
        if referance == True:
            break
        else:
            print("please enter ethier 'yes' or 'no' no other vaild option")
    if more.lower() == 'yes':
        print(df[df.columns[0:-1]].iloc[top:low])
        more_more = ''
        while more_more.lower() != 'no':
            referance2 = False
            while referance2 == False:
                more_more = input("\n get more 5 lines \n 'yes' or 'no' ")
                referance2 = get_more(more_more)
                if referance2 == True:
                    break
                else:
                    print("please enter ethier 'yes' or 'no' no other vaild option")
            if more_more.lower() == 'yes':
                top += 5
                low += 5
                print(df[df.columns[0:-1]].iloc[top:low])
            elif more_more.lower() == 'no':
                break
